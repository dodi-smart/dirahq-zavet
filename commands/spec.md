...
argument-hint: [design <feature> | backfill <feature> | blank = document this session's work]
---
...
- If a spec in `.zavet/specs/` covers it (list them via
  `sh "${CLAUDE_PLUGIN_ROOT}/bin/zavet" specs`, then read the candidates'
...
- Otherwise create `.zavet/specs/<slug>.md` (kebab-case slug, flat, from
  ...
  `${CLAUDE_PLUGIN_ROOT}/templates/spec.md` if the repo predates it) with
