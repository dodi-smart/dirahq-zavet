...
| Path | What it is |
|---|---|
| `bin/zavet` | The whole implementation. POSIX sh, ~2600 lines, no runtime deps beyond `git`/`awk`/`sed` (`jq` is a soft dep of the hooks only). |
...
Runs everything: the dialect corpus, glob matching, the CI trailer floor, audit,
verify, id collisions, the hooks, and the cross-harness adapters. There is no
watch mode and no partial run — it takes seconds.
